...
describe('get', () => {
  ...
  it('calls fetch with the expected path', () => {
    ...
    window.fetch.mockImplementationOnce((path) => Promise.resolve({ ok: true, path }))

    return expect(get('path')).resolves.toEqual(expected);
  })

  it('allows the config to be overridden', () => {
    window.fetch.mockImplementationOnce((path, config) => Promise.resolve({ ok: true, config }))
    const actual = 'config'
    const expected = {
      ok: true,
      config: actual
  ...
  it('rejects with the expected error message if fetch resolves but the response is not ok', () => {
    window.fetch.mockImplementationOnce(() => Promise.resolve({ statusText: 'The response is not ok'}))
