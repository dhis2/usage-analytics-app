import { getUsageData as getUsageDataApi } from '../api'
import * as ACTIONS from '../actions/types'
import { TOP_FAVORITES } from '../constants/categories'

export const initApp = () => async (dispatch, getState, context) => {
    try {
        const payload = await getUsageDataApi(getState().filter, context)
...
const getUsageData = filter => async (dispatch, getState, context) => {
    ...
    try {
        const usageData = await getUsageDataApi(filter, context)
