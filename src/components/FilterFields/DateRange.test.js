...
describe('<DateRange/>', () => {
    ...
    const startDateInput = wrapper.find(`[name='${START_DATE}']`)
    // const endDateInput = wrapper.find(`[name='${END_DATE}']`)
    ...
    //     startDateInput.simulate('change', { value })
    ...
    //     endDateInput.simulate('change', { value })
    //     expect(updateUsageData).toHaveBeenCalledTimes(1)
    // })
    ...
    it('renders a startDate pattern error when startDate changes into an invalid value', () => {
        ...
        startDateInput.simulate('change', { value })
        console.log('HENDRIK', wrapper.html())
        // expect(wrapper.find(`.${START_DATE} .error`)).toHaveText(ERROR_PATTERN)
        expect(
            wrapper.find('[data-test="uaa-startdate-validation"]')
        ).toHaveText(ERROR_PATTERN)
        // data-test="uaa-startdate-validation"
    })
